...
class StudentRepository:
    ...
    @staticmethod
    def _apply_filters(query: Query, *, department: Optional[str], enroll_year: Optional[int], keyword: Optional[str]) -> Query:
        # 功能：根据院系、入学年份及关键字组合过滤学生查询。
        if department:
    ...
    @classmethod
    def list(cls, *, department: Optional[str] = None, enroll_year: Optional[int] = None, keyword: Optional[str] = None, page: int = 1, per_page: int = 20) -> Tuple[List[Student], int]:
        # 功能：执行分页学生检索并返回结果集与总数。
        query = Student.query
        query = cls._apply_filters(query, department=department, enroll_year=enroll_year, keyword=keyword)
        total = query.count()
        items = (
